...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for b4bd is necessary for this upgrade with the "reindex=1" b4b.conf option. \n' +
                 'There are changes necessary in both b4b.conf and b4bcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/B4Bcoin/b4bcore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed b4bcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './b4bcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in b4bcore-node services directory
    ...
    if (servicePackage.b4bcoreNode) {
      serviceModule = service.name + '/' + servicePackage.b4bcoreNode;
    }
 ...
 * @param {Node} node - The B4bcore Node instance
 */
