...
/**
 * Will remove a service from b4bcore-node.json
...
/**
 * Will remove the Node.js service and from the b4bcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The b4bcore-node configuration directory
...
function remove(options, done) {
  ...
  var b4bcoreConfigPath = path.resolve(configPath, 'b4bcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(b4bcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a b4bcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to b4bcore-node.json
        removeConfig(b4bcoreConfigPath, service, next);
      });
    }, done
  );
}
